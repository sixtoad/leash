...
| Leash manager image     | `--leash-image`, `LEASH_IMAGE`          | Native releases default to their matching immutable `ghcr.io/sixtoad/leash-manager@sha256:...`; compatible labeled overrides remain supported. |
...
- **Linux: eBPF LSM (Layer 1) unavailable** — leash's Layer 1 enforcement is an eBPF LSM program, which only attaches when `bpf` is one of the kernel's *active* LSMs (`cat /sys/kernel/security/lsm`). Many distros ship `CONFIG_BPF_LSM=y` but don't enable it at boot. Following leash's defense-in-depth model, this is **not fatal by default**: leash warns loudly and continues with **proxy-only** enforcement (Layer 2 is fail-closed). The warning prints the fix — append `bpf` to the `lsm=` list in `GRUB_CMDLINE_LINUX` (`/etc/default/grub`), then `sudo update-grub && sudo reboot`. Pass **`--require-lsm`** (or `LEASH_REQUIRE_LSM=1`) to make a missing/unattachable Layer 1 a hard stop instead. If the kernel lacks `CONFIG_BPF_LSM` entirely, you need a kernel built with it (Linux ≥ 5.7, plus `CONFIG_DEBUG_INFO_BTF=y`).
